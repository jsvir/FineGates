...
from model import SparseLayerPretrain
...
def parse_args(args):
    ...
    parser.add_argument("--warmup_steps", type=int, default=0) # started with 10K
    ...
    parser.add_argument("--sparsity_start_step", type=int, default=1000) # equal to warmup steps
    parser.add_argument("--sparsity_lambda", type=float, default=100)
...
def evaluate_model(model, preprocess_batched, pad_idx, global_rank, world_size, device, batch_size):
    ...
    val_data = val_data.shuffle(seed=1948)
...
def main(args):
    ...
    if global_rank == 0:
        wandb.init(project="llama1b-c4-gates")
    ...
    if not args.disable_sparsity:
        for layer in model.model.layers:
                ...
                layer.self_attn.q_proj = SparseLayerPretrain(
    ...
    # ##############################
    sparse_layers = [m for m in model.modules() if isinstance(m, SparseLayerPretrain)]
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        slambda = 0 if update_step < args.sparsity_start_step else args.sparsity_lambda
        sparsity_loss = compute_sparsity_loss(sparse_layers)

        loss = model_loss + slambda * sparsity_loss
