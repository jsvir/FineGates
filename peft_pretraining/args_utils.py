...
def check_args_torchrun_main(args):
    ...
    if args.save_dir is None:
        ...
        if getattr(args, 'model_config', None) is not None:
            model_name = args.model_config.split('/')[-1].rstrip('.json')
        else:
            model_name = args.model_name.split('/')[-1].rstrip('.json')
        args.save_dir = f"checkpoints2/{model_name}-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
